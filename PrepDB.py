            ...
            if self.InSequence:
                ...
                if FileLine[:3] == "SQ ":
                    if CorrectSpecies:
                        ...
                        Str = struct.pack(
                            "<qi80s", SourceFileRecordStart, SquishedFilePos, ID)
    ...
    def Compress(self):
        ...
        while (1):
            ...
            if FileLine[0] == ">":
                ...
                ID = ID.replace("\t", " ")
                # for Ensembl chromosome format
                ID = 'chr' + ID.split(' ')[0]
                # Note: Important to call tell() *after* writing the asterisk!
                # (Fixed a bug 1/20/5)
                SquishedFilePos = self.SquishedFile.tell()
...
if __name__ == "__main__":
    ...
    # human proteins.
    Species = None
    Squasher = CompressorClass(
        SourceFileName, SquishedFileName, IndexFileName, Species)
    Squasher.Compress()
